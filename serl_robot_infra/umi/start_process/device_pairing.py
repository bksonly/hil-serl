...
class DevicePairing:
    ...
    def load_devices(self):
        ...
        # 若首次未检测到 Vive，询问用户是否为全功能版本（应有 Vive）
        if len(self.vive_serials) == 0:
            try:
                resp = input("\n未检测到任何 Vive 设备。此部署是否为全功能版本（应有 Vive）? (y/n): ").strip().lower()
            except Exception:
                resp = 'n'
            if resp in ('y', 'yes'):
                print("\n❌ 期望有 Vive，但未检测到。请检查 vive_publisher、SteamVR 和连接。")
                return False
            else:
                print("\n✅ 已确认无 Vive，流程将按仅 XV 设备继续。")
        
        # 验证设备数量（修复逻辑：仅当检测到 Vive 设备时才校验数量匹配）
        if len(self.vive_serials) > 0 and (len(self.xv_serials) != len(self.vive_serials)):
            print(f"\n❌ 错误: XV 设备数量 ({len(self.xv_serials)}) 与 Vive 设备数量 ({len(self.vive_serials)}) 不匹配")
            print("   提示: 可在无 Vive 场景下继续，仅进行 XV 录制")
            return False
    ...
    def _save_single_device_config(self):
        ...
        try:
            ...
            print(f"vive device found: {self.vive_serials[0]}")
            ...
            print(f"no vive device found: {e}")
        ...
        if self.vive_serials:
            print(f"   Vive: {self.vive_serials[0]}")
        else:
            print("   Vive: 未检测到（将按无 Vive 运行）")
        ...
        if self.vive_serials:
            for serial in self.vive_serials:
                ...
                try:
                    ...
                    print(f"  ⚠️ 跳过 Vive 订阅 {topic}: {e}")
        else:
            print("  🎮 未检测到 Vive 设备，跳过订阅")
    ...
    def analyze_pairing(self, motion_threshold=0.15, static_threshold=0.08):
        ...
        print(f"  Vive: {moving_vive if self.vive_serials else '无'}")
        
        print(f"\n静止设备:")
        print(f"  XV: {static_xv}")
        print(f"  Vive: {static_vive if self.vive_serials else '无'}")
        
        # 验证配对结果
        # 无 Vive 场景：仅返回 XV 配置，Vive 留空
        if len(self.vive_serials) == 0:
            print("\n⚠️ 无 Vive 设备，按单设备（或双 XV）保存配置")
            print("\n" + "=" * 60)
            print("✅ 配对成功!")
            print("=" * 60)
            print("\nDevice_0 (左侧 / 运动的):")
            print(f"  XV: {self.xv_serials[0]}")
            print("\nDevice_1 (右侧 / 静止的):")
            print(f"  XV: {self.xv_serials[1] if len(self.xv_serials) > 1 else self.xv_serials[0]}")
            return {
                "device_0": {
                    "label": "left_hand",
                    "xv_serial": self.xv_serials[0],
                ...
                "device_1": {
                    "label": "right_hand",
                    "xv_serial": self.xv_serials[1] if len(self.xv_serials) > 1 else self.xv_serials[0],
                    "vive_serial": ""
                }
            }

...
show_help() {
    ...
    echo "  --no-filter          启动所有检测到的设备（默认：只启动config.json中配置的设备）"
    echo "  --vive               启用Vive发布器（默认：不启动）"
    echo ""
    echo "示例:"
    echo "  $0                    # 启动服务（GUI模式，使用config.json配置）"
    echo "  $0 --no-gui           # 启动服务（后台模式）"
    echo "  $0 --no-filter        # 启动所有检测到的设备（配对前使用）"
    echo "  $0 --vive             # 启用Vive发布器"
    echo ""
    echo "说明:"
    echo "  - 默认在新终端窗口中启动服务"
    echo "  - 使用 --no-gui 在后台运行"
    echo "  - 默认只启动config.json中配置的设备"
...
start_clamp_service() {
    ...
    if [ "$NO_FILTER" = true ]; then
        ...
        devices_to_start=("${xvisio_SERIALS[@]}")
    else
        # 从config.json读取配置的设备
        if [ ! -f "$SCRIPT_DIR/config.json" ]; then
            print_error "配置文件不存在: $SCRIPT_DIR/config.json"
            print_error "请先运行配对流程或使用 --no-filter 启动所有设备"
            return 1
        fi
        
        # 读取配置的设备序列号
        devices_to_start=($(python3 -c "
import json
import sys
try:
    ...
    for key in sorted(devices.keys()):
        xv_serial = devices[key].get('xv_serial', '')
        if xv_serial:
            print(xv_serial)
...
" 2>/dev/null))
        ...
        if [ ${#devices_to_start[@]} -eq 0 ]; then
            print_error "config.json中未找到设备序列号"
            print_error "请先运行配对流程或使用 --no-filter 启动所有设备"
            return 1
        fi
        
        print_info "从config.json读取到 ${#devices_to_start[@]} 个设备: ${devices_to_start[*]}"
    fi
    ...
    # 遍历要启动的设备
    for serial in "${devices_to_start[@]}"; do
    ...
    echo ""
    print_info "夹具服务启动总结:"
    print_info "  启动设备: ${#devices_to_start[@]}"
...
main_start() {
    ...
    # 启动Vive发布器（如果启用）
    if [ "$ENABLE_VIVE" = true ]; then
        if ! start_vive_publisher; then
            print_error "Vive发布器启动失败"
            exit 1
        fi
    else
        print_info "跳过Vive发布器启动（使用 --vive 启用）"
    fi
